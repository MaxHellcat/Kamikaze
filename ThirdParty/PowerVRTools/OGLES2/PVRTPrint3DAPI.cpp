...
// Comment out for perfomance reasons
//		if (glGetError())
//		{
...
//	static GLboolean	bVertexPointerEnabled, bColorPointerEnabled, bTexCoorPointerEnabled;
	...
	case 0:
	{
//		if (!m_pAPI) printf("m_pAPI is NULL\n");
		...
		******************************/
		 ...
		 */
        Matrix.f[0] = (2.0f/(m_fScreenScale[0]*1024));
        Matrix.f[5] = (-2.0f/(m_fScreenScale[1]*768));

//		Matrix.f[4] = -(3.0f/(m_fScreenScale[0]*8.0f));
//		Matrix.f[1] = -(1.0f/(m_fScreenScale[1]*6.0f));
//		Matrix.f[7] = (1.0f); 
		Matrix.f[10] = (0.0f);
		Matrix.f[12] = (0.66f); 
		Matrix.f[13] = (1.0f);  
